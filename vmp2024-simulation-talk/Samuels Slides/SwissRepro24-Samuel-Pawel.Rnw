...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Improving the quality of simulation studies with simulation protocols},
  pdfsubject={Samuel Pawel}
}



\date{$\star$ Department of Biostatistics, Center for Reproducible Science, University of Zurich}
\title{~\\ ~ \\ \textbf{Improving the quality of simulation studies with simulation protocols}}
\author{Björn S. Siepe, Frantisek Bartos, Tim P. Morris, Anne-Laure Boulesteix, Daniel Heck, \textbf{Samuel Pawel}\textsuperscript{$\star$}}
\institute{
  June 10, 2024, Swiss Reproducibility Conference 2024
}
\titlegraphic{
  \includegraphics[height=1.75em]{pics/UZHlogo.pdf}
  \hspace{0.2em}
  \includegraphics[height=2em]{pics/crs1.png}
  \hfill
  \includegraphics[height=1.75em]{pics/snf.png}
}
...
\begin{frame}{Evidence from simulation studies}
  \only<1>{
    \begin{block}{}
      \begin{tcolorbox}[colframe=chineseBlue% , title=\textbf{Abstract}
        ]
  ...
  \only<2>{
    \begin{columns}
      ...
      \begin{column}{0.35\textwidth}
        \begin{block}{}
          \centering
          % \includegraphics[width=0.8\linewidth,frame]{pics/BradleyEfffron.png}
          % {\tiny \color{gray} \href{https://en.wikipedia.org/wiki/Bradley_Efron}{https://en.wikipedia.org/wiki/Bradley\_Efron}}
...
\begin{frame}{Conclusions}
  ...
  {\tiny \color{gray} \href{https://doi.org/10.31234/osf.io/ufgy6}{doi.org/10.31234/osf.io/ufgy6}}
\end{frame}
